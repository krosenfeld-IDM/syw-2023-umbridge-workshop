...
r = requests.post(f"{args.url}/InputSizes", json={"name": name})
rInputSizes = r.json()
print(rInputSizes)


print("Requesting output sizes...")
r = requests.post(f"{args.url}/OutputSizes", json={"name": name})
print(r.text)
...
inputParams = {"name": name, "input": [], "config": {}}
for i in range(0,len(rInputSizes["inputSizes"])):
  inputParams["input"].append([np.random.rand()] * rInputSizes["inputSizes"][i])
print(inputParams)
