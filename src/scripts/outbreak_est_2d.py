...
def plot_Z_dist(m, s, ax, emod_results=None, pymc_results=None, 
    n=10000, bins=np.linspace(0,1), color=methods.DefaultColors().emod):
    """
    ...
    if emod_results is not None:
        ax.hist(emod_results["atk_frac"], bins=bins, histtype="step", color=color, 
            lw=1, density=True)
...
emod_results = methods.DataBrick(tpath = paths.emod / "model_covariance01" / "experiment_workshop_2d_00").to_df()
...
param_levels = emod_results[param_keys].drop_duplicates().iloc[::-1].reset_index(drop=True)
cols = methods.DefaultColors().sensitivity
...
for iy in range(len(R0_ranges)):
    ...
    for ix, (col, (_, params)) in enumerate(zip(cols, param_levels.iterrows())):

        # create axes
        ax = plt.axes(fc.get_rect(ix+1,iy))

        # get EMOD results    
        select_emod_results = pd.merge(emod_results[emod_results["R0_name"] == name],
            pd.DataFrame(params).T, on=param_keys)

        # plot
        plot_Z_dist(m,s,ax,select_emod_results,color=col)    
...
plt.savefig(paths.figures / 'outbreak_est_2d_R0.png', transparent=1)
